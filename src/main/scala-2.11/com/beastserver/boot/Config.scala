...
trait Config {
  ...
  //Beast settings
  object BeastDefaults {
    lazy val beastHost: String = Try {
      factory.getString("beast.host")
    } filter {
      _.nonEmpty
    } get

    lazy val beastPort: Int = Try {
      factory.getInt("beast.port")
    } filter {
      _ > 0
    } get
    lazy val receiveTimeout: Duration = (Try {
      factory.getInt("beast.receiveTimeout")
    } filter {
      _ > 0
    } get).seconds
    lazy val pageSize: Int = Try {
      factory.getInt("beast.pageSize")
    } filter {
      _ > 0
    } getOrElse 10
    lazy val bookSize: Int = Try {
      factory.getInt("beast.bookSize")
    } filter {
      _ > 0
    } getOrElse 100
  }
